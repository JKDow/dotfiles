-- Leader is <space>
vim.g.mapleader = " "
-- Return to Netrw
vim.keymap.set("n", "<leader>pv", vim.cmd.Ex)
-- Move current line up/down
vim.keymap.set("v", "J", ":m '>+1<CR>gv=gv")
vim.keymap.set("v", "K", ":m '<-2<CR>gv=gv")
-- Move line below to end of current line
vim.keymap.set("n", "J", "mzJ`z")
-- Half page jumps with re-center
vim.keymap.set("n", "<C-d>", "<C-d>zz")
vim.keymap.set("n", "<C-u>", "<C-u>zz")
-- Search jumps open folds and center window view
vim.keymap.set("n", "n", "nzzzv")
vim.keymap.set("n", "N", "Nzzzv")
-- Paste over without changing register
vim.keymap.set("x", "<leader>p", [["_dP]])
-- Copy to system clipboard
vim.keymap.set({ "n", "v" }, "<leader>y", [["+y]])
vim.keymap.set("n", "<leader>Y", [["+Y]])
-- delete but dont save to register
vim.keymap.set({ "n", "v" }, "<leader>d", [["_d]])
-- Disable Ex mode
vim.keymap.set("n", "Q", "<nop>")
-- Tmux Sessionizer
vim.keymap.set("n", "<C-f>", "<cmd>silent !tmux neww tmux-sessionizer<CR>")
-- Quickfix list and location list hotkeys
vim.keymap.set("n", "<C-k>", "<cmd>cnext<CR>zz")
vim.keymap.set("n", "<C-j>", "<cmd>cprev<CR>zz")
vim.keymap.set("n", "<leader>k", "<cmd>lnext<CR>zz")
vim.keymap.set("n", "<leader>j", "<cmd>lprev<CR>zz")
-- Search and replace
vim.keymap.set(
    "n",
    "<leader>s",
    [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]]
)
-- Set current file as executable
vim.keymap.set("n", "<leader>x", "<cmd>!chmod +x %<CR>", { silent = true })
-- Unset the shift down binds
vim.api.nvim_set_keymap('n', '<S-Down>', '<NOP>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('i', '<S-Down>', '<NOP>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', '<S-Up>', '<NOP>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('i', '<S-Up>', '<NOP>', { noremap = true, silent = true })
-- paste from clipboard (relies on xclip and xsel)
vim.keymap.set('n', '<leader>P', function()
    vim.opt.paste = true
    vim.cmd('normal! "+p')
    vim.opt.paste = false
end, { noremap = true, silent = true })
